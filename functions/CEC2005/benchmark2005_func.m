function f=benchmark2005_func(x,func_num)
global initial_flag 
...
x=x-repmat(o,ps,1); % ps 是个体数目, x-repmat,将整个solution往右移动（左加右减，上加下减）
fit=sum(x.^2,2); % 2表示每一行求和
...
% classical Gram Schmid 
 function [q,r] = cGram_Schmidt (A) % Gram_Schmidt正交
...
A=normrnd(0,1,D,D); % 正态随机数数组
P=cGram_Schmidt(A); % 正交基？
A=normrnd(0,1,D,D);
Q=cGram_Schmidt(A);
u=rand(1,D);
