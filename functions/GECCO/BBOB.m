...
%
% see also: functions FGENERIC, BENCHMARKINFOS, BBOBNOISY
...
%%%-------------------------------------------------------------%%%
function [res, res2] = BBOB(x, strfun, DIM) 
%
  Nfcts = 24;
  ...
  if nargin < 1 || ( ...
    ...
    for i = 1:100
      try  % exist does not work under Octave
        ...
        if i < Nfcts
          fprintf('execution of function %d produces an error', i);
        end
      end
    end 
    if length(res) > Nfcts
